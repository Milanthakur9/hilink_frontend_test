...
function Page() {
  ...
  useEffect(() => {
    ...
    const fetchEventData = async () => {
      try {
        const response = await axiosInstance.get("/v1/event/info/2"); // Fetch data from the endpoint
        setEventData(response.data.data); // Update state with the fetched data
          ...
          >
            ...
            >
              <Box sx={{ width: { md: "50%", xs: "100%" } }}>
                ...
                >
                  Events by {eventData?.eventName}
                </Typography>
                <Typography variant="h1" sx={{ fontWeight: "bold" }}>
                  The Santa Con Christmas Party
                </Typography>
                <Typography sx={{ fontSize: "20px", margin: "2% 0 4% 0" }}>
                  HK Hall <br></br> {eventData?.startDateAndTime} -{" "}
                  {eventData?.endDateAndTime}
                </Typography>
                ...
                <Typography>
                  {/* Santa Con Christmas 2024 Festival 1500+ in attendance : Santa
                  Hats Encouraged ; Sounds By: Dj Exxtra , TDR & More */}
        ...
        >
          <Box sx={{ margin: "4% 0" }}>
            ...
            >
              ...
              >
                <Typography
                  sx={{ textDecoration: `line-through solid ${white}` }}
                >
                  Pre Sale
                </Typography>
              ...
              {/* buy tickets start  */}
              <Buy
                title="General Admission ( Will be sold out )"
                price={16}
                subtitle="Will be sold out get your tickest ASAP"
              />
              {/* buy tickets end  */}

              {/* buy tickets start  */}
              <Buy
                title="GA + Santa Hat ( Will be sold out !)"
                price={18}
                subtitle="General Admission + Santa Hat"
              />
              {/* buy tickets end  */}

              {/* buy tickets start  */}
              <Buy
                title="Ladies Group Order Good for 3"
                price={45}
                subtitle="Good for 3 Ladies"
              />
              {/* buy tickets end  */}

              {/* buy tickets start  */}
              <Buy
                title="VIP BALCONY TABLE & BOTTLE DM @212Groupnyc on Insta"
                price={350}
                subtitle="500Minimum spend + Tax & Gratuity DM @212Groupnyc"
              />
              {/* buy tickets end  */}
            </Box>
          </Box>
          ...
          <Box sx={{ width: { md: "70%", xs: "95%" }, margin: "7% auto" }}>
            <iframe
              ...
              width="957"
              height="402"
              src="https://www.youtube.com/embed/uWsLkHcYpPo"
              title="Y2K Halloween Party @ The Brooklyn Monarch"
          ...
          <Box>
            {typoData.map((item) => {
              return (
                <Typography
                  variant="subtitle1"
                  sx={{ margin: "10px 0px", opacity: 0.7 }}
                  key={item.name}
                >
                  {item.name}
                </Typography>
          ...
          <Box>
            ...
            >
              {imageList.map((item, i) => {
                return (
                  <Box
                    sx={{ width: "100px", height: "100px", marginTop: "10px" }}
                    key={i}
                  >
                    <Image
                      style={{
          ...
          <Box sx={{ margin: "5% 0 2% 0" }}>
            ...
            ></Divider>
            <Typography variant="h3" sx={{ margin: "1% 0 4% 0" }}>
              PERFORMANCES
            </Typography>
          </Box>
          ...
          <Box>
            ...
            <Button
              startIcon={<PlaceIcon />}
              sx={{ background: orange, color: white, paddingInline: "20px" }}
            >
              OPEN IN MAPS
            </Button>
          </Box>
